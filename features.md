...
- [x] offline logging: IndexedDB-backed mutation queue for log entries (create/edit/delete) and exercise/variation edits, optimistic local results (negative temp ids for entries created offline), auto-sync on reconnect + manual "Sync now", app-bar pending badge + offline chip, service worker app-shell caching for a cold open with no connectivity, System page "Offline sync" panel for queued/failed items
- [x] safe-area insets for notched/rounded-corner phones on both the app bar (top) and bottom nav, pure CSS (content-box + env()), works in a regular mobile browser tab as well as installed-PWA standalone mode - third attempt at the bottom nav specifically, after two prior tries were parked; see global.css comments for why this one should hold up
- [x] audit pass: Home/exercise-detail bound their log-entry fetch to the last 180 days instead of all-time (Stats' Records tab and Log's search stay full-history, deliberately); discarding a queued offline mutation now refetches instead of leaving the optimistic edit stranded; a queued mutation that fails repeatedly for non-connectivity reasons gets marked failed after 3 tries instead of silently blocking every mutation queued behind it forever; any unhandled API/store error now surfaces as a snackbar instead of failing silently in the console; added missing aria-labels to icon-only buttons
- [x] ergonomics pass: the Home "+&lt;value&gt;" quick-log button shows the actual reps/time as visible text instead of icon+tooltip; the rep stepper's number is now a tap-to-edit field alongside the +/- buttons, for higher-rep sets where tapping + repeatedly was slow; buttons no longer trigger a double-tap-zoom on quick repeated taps; installable-PWA long-press shortcuts for your 3 most-recently-logged working variations, showing the value that'll be logged, deep-linking straight into the quick-log sheet (Android/Chrome only - iOS Safari doesn't support manifest shortcuts); one shared top-of-page loading bar instead of each view owning its own; every snackbar (log confirmation, undo, sync status, errors) now goes through one shared queue instead of separate ones clobbering each other; audited every delete/remove action for a confirm step or undo (added a confirm dialog for discarding a queued offline change, undo for removing a routine item, explicit cascade-delete warning on deleting an exercise); settled on "working variation" as the term in all user-facing copy (the `isFavorite` field/API name is unchanged)
- [x] TRMNL e-ink dashboard support: public `GET /api/trmnl` route (under `/api` so it works behind a reverse proxy that only forwards that prefix; explicitly excluded from the passcode gate) returns a static 800x480 HTML page (no client JS) listing the most-overdue working variations, sized/styled for TRMNL's screenshot-based "private plugin" polling; the overdue-favorites ranking logic is now shared with the Discord idle reminder instead of duplicated (lib/overdueFavorites.ts)
- [x] standalone Timer page (`/timer`, plain Start/Stop/Reset stopwatch, nothing gets logged) reachable from a clock icon in the app bar on every page; moved the System link off the app bar (freed up that icon slot) to a button on Manage exercises instead - the offline pending-sync count is now a small chip in the app bar linking to System rather than a badge on the old icon
- [x] Discord idle reminder: cron default changed from every 5 minutes to hourly (the once-per-idle-stretch cooldown already prevented repeats, the tight schedule wasn't doing anything); the cooldown now also lifts on a new calendar day even without a new set logged, so several fully-inactive days each get one reminder instead of going silent after the first one ever sent
